...
from utils.eval_t2m import evaluation_mask_transformer_con6, evaluation_res_transformer_con
...
class MaskTransformerTrainer:
    def __init__(self, args, t2m_transformer, left_arm_vq_model, right_arm_vq_model, up_body_vq_model, left_leg_vq_model, right_leg_vq_model, other_body_vq_model):
        self.opt = args
    ...
    def forward(self, batch_data):
        ...
        # (b, n, q)
        left_arm_code_idx, _ = self.left_arm_vq_model.encode(motion[:, :, self.opt.left_arm_dim])
        right_arm_code_idx, _ = self.right_arm_vq_model.encode(motion[:, :, self.opt.right_arm_dim])
        up_body_code_idx, _ = self.up_body_vq_model.encode(motion[:, :, self.opt.up_body_dim])
        left_leg_code_idx, _ = self.left_leg_vq_model.encode(motion[:, :, self.opt.left_leg_dim])
        right_leg_code_idx, _ = self.right_leg_vq_model.encode(motion[:, :, self.opt.right_leg_dim])
        other_body_code_idx, _ = self.other_body_vq_model.encode(motion[:, :, self.opt.other_body_dim])

        b, n, q = left_arm_code_idx.shape
        # 在新维度上堆叠它们
        code_idx = torch.zeros((b, n*6, q)).cuda()

        code_idx[:, 0::6] = left_arm_code_idx
        code_idx[:, 1::6] = up_body_code_idx
        code_idx[:, 2::6] = left_leg_code_idx
        code_idx[:, 3::6] = right_leg_code_idx
        code_idx[:, 4::6] = other_body_code_idx
        code_idx[:, 5::6] = right_arm_code_idx
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval):
        ...
        best_fid, best_div, best_top1, best_top2, best_top3, best_matching, writer = evaluation_mask_transformer_con6(
            self.opt, self.opt.save_root, eval_val_loader, self.t2m_transformer, self.left_arm_vq_model,
            self.right_arm_vq_model, self.up_body_vq_model, self.left_leg_vq_model, self.right_leg_vq_model, self.other_body_vq_model, self.logger, epoch, best_fid=100, best_div=100,
        ...
        while epoch < self.opt.max_epoch:
            ...
            best_fid, best_div, best_top1, best_top2, best_top3, best_matching, writer = evaluation_mask_transformer_con6(
                self.opt, self.opt.save_root, eval_val_loader, self.t2m_transformer, self.left_arm_vq_model,
                self.right_arm_vq_model, self.up_body_vq_model, self.left_leg_vq_model, self.right_leg_vq_model, self.other_body_vq_model, self.logger, epoch,
...
class ResidualTransformerTrainer:
    def __init__(self, args, res_transformer, left_arm_vq_model, right_arm_vq_model, up_body_vq_model, down_body_vq_model):
        self.opt = args
    ...
    def forward(self, batch_data):
        ...
        left_arm_code_idx, _ = self.left_arm_vq_model.encode(motion[:, :, self.opt.left_arm_dim])
        up_body_code_idx, _ = self.up_body_vq_model.encode(motion[:, :, self.opt.up_body_dim])
        down_body_code_idx, _ = self.down_body_vq_model.encode(motion[:, :, self.opt.down_body_dim])
        right_arm_code_idx, _ = self.right_arm_vq_model.encode(motion[:, :, self.opt.right_arm_dim])

        b, n, q = left_arm_code_idx.shape
        # 在新维度上堆叠它们
        code_idx = torch.zeros((b, n*4, q)).cuda().to(torch.int64)

        code_idx[:, 0::4] = left_arm_code_idx
        code_idx[:, 1::4] = up_body_code_idx
        code_idx[:, 2::4] = down_body_code_idx
        code_idx[:, 3::4] = right_arm_code_idx
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval):
        ...
        best_fid, best_div, best_top1, best_top2, best_top3, best_matching, writer = evaluation_res_transformer_con(
            self.opt, self.opt.save_root, eval_val_loader, self.res_transformer, self.left_arm_vq_model,
            self.right_arm_vq_model, self.up_body_vq_model, self.down_body_vq_model, self.logger, epoch,
        ...
        while epoch < self.opt.max_epoch:
            ...
            best_fid, best_div, best_top1, best_top2, best_top3, best_matching, writer = evaluation_res_transformer_con(
                self.opt, self.opt.save_root, eval_val_loader, self.res_transformer, self.left_arm_vq_model,
                self.right_arm_vq_model, self.up_body_vq_model, self.down_body_vq_model, self.logger, epoch, best_fid=best_fid,
