...
from utils.eval_t2m import evaluation_vqvae_seg
...
class RVQTokenizerTrainer:
    def __init__(self, args, left_arm_vq_model, right_arm_vq_model, body_vq_model):
        self.opt = args
    ...
    def train_batch(self, batch_data, vq_model, opt_vq_model, it, scheduler, logs, name):

        motions = batch_data.detach().to(self.device).float()
        pred_motion, loss_commit, perplexity = vq_model(motions)
        
        self.motions = motions
        self.pred_motion = pred_motion

        loss_rec = self.l1_criterion(pred_motion, motions)
        pred_local_pos = pred_motion[1:] - pred_motion[:-1]
        local_pos = motions[1:] - motions[:-1]
        ...
        logs[name+'loss'] += loss.item()
        logs[name+'loss_rec'] += loss_rec.item()
        # Note it not necessarily velocity, too lazy to change the name now
        logs[name+'loss_vel'] += loss_explicit.item()
        logs[name+'loss_commit'] += loss_commit.item()
        logs['perplexity'] += perplexity.item()
        logs['lr'] += opt_vq_model.param_groups[0]['lr']


    def eval_batch(self, batch_data, left_arm_vq_model, right_arm_vq_model, body_vq_model):
        motions = batch_data.detach().to(self.device).float()
        ...
        pre_body, _, _ = body_vq_model(motions[:, :, self.opt.whole_body_dim])
        ...
        pred_motion[:, :, self.opt.whole_body_dim] = pre_body
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval=None):
        ...
        # sys.exit()
        best_fid, best_div, best_top1, best_top2, best_top3, best_matching, writer = evaluation_vqvae_seg(
            self.opt, self.opt.model_dir, eval_val_loader, self.left_arm_vq_model, self.right_arm_vq_model,
            self.body_vq_model, self.logger, epoch, best_fid=1000, best_div=100, best_top1=0,
        ...
        while epoch < self.opt.max_epoch:
            ...
            for i, batch_data in enumerate(train_loader):
                ...
                self.train_batch(batch_data[:, :, self.opt.left_arm_dim], self.left_arm_vq_model, self.opt_left_arm_vq_model, it, 
                                 self.left_arm_scheduler, logs, 'leftarm')
                self.train_batch(batch_data[:, :, self.opt.right_arm_dim], self.right_arm_vq_model, self.opt_right_arm_vq_model, it, 
                                 self.right_arm_scheduler,logs, 'rightarm')
                self.train_batch(batch_data[:, :, self.opt.whole_body_dim], self.body_vq_model, self.opt_body_vq_model, it, 
                                 self.body_scheduler, logs, 'body')  
            ...
            with torch.no_grad():
                for i, batch_data in enumerate(val_loader):
                    loss_rec = self.eval_batch(batch_data, self.left_arm_vq_model, self.right_arm_vq_model, self.body_vq_model)
            ...
            best_fid, best_div, best_top1, best_top2, best_top3, best_matching, writer = evaluation_vqvae_seg(
                self.opt, self.opt.model_dir, eval_val_loader, self.left_arm_vq_model, self.right_arm_vq_model, self.body_vq_model, self.logger, epoch, best_fid=best_fid,
