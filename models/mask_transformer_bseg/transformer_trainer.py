...
from utils.eval_t2m import evaluation_mask_transformer_seg, evaluation_res_transformer_bseg
...
class MaskTransformerTrainer:
    def __init__(self, args, t2m_transformer, left_arm_vq_model, right_arm_vq_model, body_vq_model):
        self.opt = args
    ...
    def forward(self, batch_data):
        ...
        body_code_idx, _ = self.body_vq_model.encode(motion[:, :, self.opt.whole_body_dim])

        b, n, q = left_arm_code_idx.shape
        # 在新维度上堆叠它们
        code_idx = torch.zeros((b, n*3, q)).cuda()

        code_idx[:, 0::3] = left_arm_code_idx
        code_idx[:, 1::3] = body_code_idx
        code_idx[:, 2::3] = right_arm_code_idx
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval):
        ...
        best_fid, best_div, best_top1, best_top2, best_top3, best_matching, writer = evaluation_mask_transformer_seg(
            ...
            self.right_arm_vq_model, self.body_vq_model, self.logger, epoch, best_fid=100, best_div=100,
        ...
        while epoch < self.opt.max_epoch:
            ...
            best_fid, best_div, best_top1, best_top2, best_top3, best_matching, writer = evaluation_mask_transformer_seg(
                self.opt, self.opt.save_root, eval_val_loader, self.t2m_transformer, self.left_arm_vq_model,
                self.right_arm_vq_model, self.body_vq_model, self.logger, epoch,
...
class ResidualTransformerTrainer:
    def __init__(self, args, res_transformer, left_arm_vq_model, right_arm_vq_model, body_vq_model):
        self.opt = args
    ...
    def forward(self, batch_data):
        ...
        b, n, q = left_arm_code_idx.shape
        # 在新维度上堆叠它们
        code_idx = torch.zeros((b, n*3, q)).cuda().to(torch.int64)
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval):
        ...
        best_fid, best_div, best_top1, best_top2, best_top3, best_matching, writer = evaluation_res_transformer_bseg(
        ...
        while epoch < self.opt.max_epoch:
            ...
            best_fid, best_div, best_top1, best_top2, best_top3, best_matching, writer = evaluation_res_transformer_bseg(
                self.opt, self.opt.save_root, eval_val_loader, self.res_transformer, self.left_arm_vq_model,
                self.right_arm_vq_model, self.body_vq_model, self.logger, epoch, best_fid=best_fid,
