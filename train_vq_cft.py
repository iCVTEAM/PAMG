...
from models.vq_cft.model import RVQVAE, CFineTune
from models.vq_cft.vq_trainer import RVQTokenizerTrainer
...
if __name__ == "__main__":
    ...
    if opt.dataset_name == "t2m":
        ...
        dim_pose = 251
        opt.left_arm_dim, opt.right_arm_dim, opt.up_body_dim, opt.down_body_dim = dim_sep(opt.dataset_name)
        left_arm_exp, right_arm_exp, up_body_exp, down_body_exp = 27, 27, 9, 179
    ...
    left_arm_pc_vq = sum(param.numel() for param in left_arm_net.parameters())
    ...
    right_arm_pc_vq = sum(param.numel() for param in right_arm_net.parameters())
    ...
    down_body_net = RVQVAE(opt,
                down_body_exp,
                opt.nb_code,
                opt.down_body_code_dim,
                opt.down_body_code_dim,
                opt.down_t,
                opt.stride_t,
                opt.width,
    ...
    finetune_net = CFineTune(dim_pose,
                             nres=3,
    ...
    trainer = RVQTokenizerTrainer(opt, left_arm_vq_model=left_arm_net, right_arm_vq_model=right_arm_net, up_body_vq_model=up_body_net, down_body_vq_model=down_body_net, fine_net=finetune_net)
