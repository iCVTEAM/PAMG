...
from models.mask_transformer_com.transformer import MaskTransformer
from models.mask_transformer_com.transformer_trainer import MaskTransformerTrainer
from models.vq_com.model import RVQVAE
...
def load_vq_model():
    ...
    ckpt = torch.load(pjoin(vq_opt.checkpoints_dir, vq_opt.dataset_name, vq_opt.name, 'model', 'net_best_fid.tar'),
                            map_location='cpu')
    body_net.load_state_dict(ckpt['body_vq_model'])
    print(f'Loading VQ Model {opt.vq_name}')
    return body_net, vq_opt
...
if __name__ == '__main__':
    ...
    body_net, vq_opt = load_vq_model()

    vq_opt.left_arm_dim, vq_opt.right_arm_dim, vq_opt.up_body_dim, vq_opt.down_body_dim = opt.left_arm_dim, opt.right_arm_dim, opt.up_body_dim, opt.down_body_dim
    ...
    if opt.fix_token_emb:
        t2m_transformer.load_and_freeze_token_emb(body_net.quantizer.codebooks[0])
    ...
    trainer = MaskTransformerTrainer(opt, t2m_transformer, body_net)
