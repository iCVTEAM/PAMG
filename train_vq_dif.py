...
from models.vq_diffusion.model import RVQVAE_DIF
from models.vq_diffusion.vq_diffusion_trainer import RVQDiffusionTrainer
...
def load_vq_model():
    ...
    return left_arm_vq_model, right_arm_vq_model, body_vq_model
...
if __name__ == "__main__":
    ...
    if opt.dataset_name == "t2m":
        ...
        opt.joints_num = 22
        dim_pose = 263
        opt.left_arm_dim, opt.right_arm_dim, opt.whole_body_dim = dim_sep(opt.dataset_name)
    ...
    left_arm_net_dif = RVQVAE_DIF(opt,
                left_arm_exp,
                opt.nb_code,
    ...
    right_arm_net_dif = RVQVAE_DIF(opt,
                right_arm_exp,
                opt.nb_code,
    ...
    # print("Total parameters of discriminator net: {}".format(pc_vq))
    # all_params += pc_vq_dis
    left_arm_net, right_arm_net, body_net = load_vq_model()

    trainer = RVQDiffusionTrainer(opt, left_arm_vq_model=left_arm_net, right_arm_vq_model=right_arm_net, body_vq_model=body_net,
                                  left_arm_vq_dif=left_arm_net_dif, right_arm_vq_dif=right_arm_net_dif, body_vq_dif=body_net_dif)
