...
from models.mask_transformer_bseg.transformer import MaskTransformer, ResidualTransformer
...
def load_vq_model(vq_opt):
    ...
    if vq_opt.dataset_name == 't2m':
        left_arm_exp, right_arm_exp, whole_body_exp = 27, 27, 209
    else:
        left_arm_exp, right_arm_exp, whole_body_exp = 27, 27, 197
...
def load_trans_model(vq_opt, model_opt, which_model):
    ...
    ckpt = torch.load(pjoin(model_opt.checkpoints_dir, model_opt.dataset_name, model_opt.name, 'model', 'net_best_fid.tar'),
...
if __name__ == '__main__':
    ...
    left_arm_net, right_arm_net, body_net, vq_opt = load_vq_model(vq_opt)
    model_opt.left_arm_dim, model_opt.right_arm_dim, model_opt.whole_body_dim = dim_sep(vq_opt.dataset_name)
    ...
    model_opt.body_code_dim = vq_opt.body_code_dim
        ...
        for i in range(repeat_time):
            with torch.no_grad():
                best_fid, best_div, Rprecision, best_matching, best_mm = \
                    eval_t2m.evaluation_mask_transformer_test_plus_res_bseg(model_opt, eval_val_loader, left_arm_net, body_net, right_arm_net, res_model, t2m_transformer,
